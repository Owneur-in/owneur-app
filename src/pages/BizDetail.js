...
export default function BizDetail({
  ...
  async function rateResponse(responded) {
    if (!user || responseRated) return;
    try {
      await supabase
        .from('response_ratings')
        .insert({ business_id: biz.id, user_id: user.id, responded });
      setResponseRated(true);
      showToast(
        responded ? 'Thanks! Marked as responsive.' : 'Thanks for the feedback.'
      );
    } catch (e) {
      setResponseRated(true);
  ...
  function callSeller() {
    if (biz.phone) {
      ...
      supabase
        .from('businesses')
        .update({ call_clicks: (biz.call_clicks || 0) + 1 })
        .eq('id', biz.id);
    } else {
      showToast('No call number provided');
    }
  }
  ...
  function openWhatsApp() {
    if (biz.whatsapp) {
      ...
      supabase
        .from('businesses')
        .update({ whatsapp_clicks: (biz.whatsapp_clicks || 0) + 1 })
        .eq('id', biz.id);
    } else {
      showToast('No WhatsApp number provided');
    }
  }

  function shareProfile() {
    const url = 'https://owneur.in/' + biz.profile_url;
    if (navigator.share) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ padding: 16 }}>
        ...
        >
          <h2
            style={{ fontSize: 20, fontWeight: 700, flex: 1, marginRight: 8 }}
          >
            {biz.name}
          </h2>
        ...
        >
          {biz.active_since && (
            ...
            >
              Active since{' '}
              {new Date(biz.active_since).toLocaleDateString('en-IN', {
                month: 'short',
                year: 'numeric',
              })}
            </div>
          )}
          {biz.response_count > 0 && (
            <div
              style={{
                background: '#DDF4EC',
                borderRadius: 20,
            ...
            >
              {biz.response_rate}% response rate
            </div>
          )}
        </div>
        ...
        <div className="card" style={{ marginBottom: 10 }}>
          ...
          <p style={{ fontSize: 20, fontWeight: 700, color: '#0A6B52' }}>
            {biz.price_range}
          </p>
        </div>
        ...
        <div className="action-row">
          <button className="action-btn call" onClick={callSeller}>
            <svg viewBox="0 0 24 24">
              <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-3.07-8.7A2 2 0 012 .99h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7a2 2 0 011.72 2.02z" />
            </svg>
            Call
          </button>
          ...
          <button className="action-btn" onClick={toggleSave}>
            <svg
              viewBox="0 0 24 24"
              style={{
                fill: isSaved ? '#E03535' : 'none',
                stroke: isSaved ? '#E03535' : 'currentColor',
              }}
            >
        ...
        <div
          className="card"
          style={{ marginBottom: 14, background: '#F2F6F4' }}
        >
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    >
                      {r.profiles ? r.profiles.full_name[0] : '?'}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div
                        style={{
                      ...
                      <p
                        style={{ fontSize: 13, color: '#516B61', marginTop: 3 }}
                      >
                        {r.review_text}
                      </p>
                    </div>
