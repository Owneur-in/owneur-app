/**
 * Search.js — Customer search with filters.
 * Category chips loaded from database — respects admin hide/show.
 */
...
export default function Search({ nav, setSelectedBiz }) {
  ...
  // Load active categories from DB on mount
  useEffect(() => {
    loadCategories()
  }, [])
  ...
  // Chips = All + active categories from DB + Verified
  const chips = ['All', ...categories.map(c => c.name), 'Verified']

  return (
    <div style={{ minHeight: '100vh', background: '#F2F6F4', paddingBottom: 80 }}>
      {/* Search bar */}
      <div style={{ background: '#fff', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '0.5px solid rgba(0,0,0,0.07)', position: 'sticky', top: 0, zIndex: 100 }}>
        <button className="back-btn" onClick={() => nav('home')}>←</button>
        <div style={{ flex: 1, background: '#F2F6F4', borderRadius: 12, display: 'flex', alignItems: 'center', padding: '8px 12px', gap: 8 }}>
          ...
          />
          {query && (
            <button style={{ border: 'none', background: 'none', color: '#516B61', cursor: 'pointer', fontSize: 16 }}
              onClick={() => setQuery('')}>✕</button>
          )}
        </div>
        <button className="btn btn-sm" style={{ flexShrink: 0, padding: '8px 12px' }}
      ...
      {/* Filter panel */}
      {showFilter && (
        <div style={{ background: '#fff', padding: '14px 16px', borderBottom: '0.5px solid rgba(0,0,0,0.07)' }}>
          <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>Sort By</p>
          <div style={{ display: 'flex', gap: 8 }}>
            {[{ val: 'newest', label: 'Newest First' }, { val: 'rating', label: 'Most Popular' }].map(opt => (
              <button key={opt.val} onClick={() => { setSortBy(opt.val); setShowFilter(false) }}
      ...
      <div style={{ padding: '0 14px 14px' }}>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <button className="btn btn-primary" onClick={() => { setQuery(''); setActiveChip('All') }}>
              Clear Filters
            </button>
          </div>
        ) : (
          results.map(biz => (
            <div key={biz.id} onClick={() => openBiz(biz)}
              style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.07)', borderRadius: 16, padding: '12px 14px', marginBottom: 8, display: 'flex', gap: 12, alignItems: 'flex-start', cursor: 'pointer' }}>
              <div style={{ width: 64, height: 64, borderRadius: 12, background: CAT_BG[biz.category] || 'linear-gradient(135deg,#F1EFE8,#D3D1C7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, flexShrink: 0 }}>
                {CAT_EMOJI[biz.category] || '🔧'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 4 }}>
                  <p style={{ fontWeight: 700, fontSize: 14 }}>{biz.name}</p>
                  {biz.is_verified && <span className="verified-badge" style={{ fontSize: 10, flexShrink: 0 }}>✓ Verified</span>}
                </div>
                <p style={{ fontSize: 12, color: '#516B61', marginBottom: 5 }}>
                  by {biz.profiles?.full_name || 'Seller'} · {biz.location_area}
                </p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: '#0A6B52' }}>{biz.price_range}</p>
                  {biz.active_since && (
                    <span style={{ fontSize: 10, color: '#516B61' }}>
                      Since {new Date(biz.active_since).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                    </span>
                  )}
                </div>
              </div>
